...
$ProjectDir = Join-Path $RootDir "pet-advisor"
...
if (Test-Path $VenvDir) {
    ...
    if ((Test-Path $cfg) -and (Select-String -Path $cfg -Pattern "version = 3\.12\." -Quiet)) {
        Write-Host "Existing root .venv already uses Python 3.12." -ForegroundColor Green
    } else {
        Write-Host "Removing existing root .venv because it is not Python 3.12..." -ForegroundColor Yellow
        Remove-Item -LiteralPath $VenvDir -Recurse -Force
    }
}

if (-not (Test-Path $VenvDir)) {
    Write-Host "Creating root .venv with Python 3.12..." -ForegroundColor Cyan
