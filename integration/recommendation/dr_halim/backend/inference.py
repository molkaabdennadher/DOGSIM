...
def has_voice_activity(audio_path: str,
                       energy_thr: float = 0.001,
                       min_dur: float = 0.15) -> bool:
    """Uses soundfile instead of librosa.load
    (Python 3.13 compatible — aifc was removed).
    """
    try:
        import soundfile as sf
        import scipy.signal as sps

        data, sr_orig = sf.read(audio_path, dtype='float32', always_2d=True)
        y = data.mean(axis=1)

        if sr_orig != 22050:
            num_samples = int(len(y) * 22050 / sr_orig)
            y = sps.resample(y, num_samples)
        sr = 22050

        if len(y) == 0:
            print("[audio] Audio buffer is empty.")
            return False

        peak = float(np.max(np.abs(y)))
        if peak < 1e-7:
            print("[audio] Audio is completely silent.")
            return False

        rms = librosa.feature.rms(y=y, frame_length=2048, hop_length=512)[0]
        voiced = float((rms > energy_thr).sum() * 512 / sr)
        print(f"[audio] peak={peak:.5f} RMS_max={rms.max():.5f} "
              f"voiced_dur={voiced:.2f}s thr={energy_thr} min_dur={min_dur}")
...
def wav_to_melspec(audio_path: str):
    """Load audio using soundfile (Python 3.13 compatible)."""
    import soundfile as sf
    import scipy.signal as sps
